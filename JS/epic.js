/* =========================================
   ...
   ========================================= */
(() => {
  // ✅ Segmento épico real: 2:25 → 3:59 → 145s a 239s
  const EPIC_START = 145;
  const EPIC_END   = 239;

  // Compases relativos al inicio del segmento (en segundos)
  const compasRelativo = [
    0, 3.6, 7.2, 10.8, 14.4, 18, 21.6, 25.2,
    28.8, 32.4, 36, 39.6, 43.2, 46.8, 50.4, 54
  ];

  // Convertimos a tiempos absolutos en la pista completa
  ...
  function init() {
    ...
    if (!audio) {
      console.error("❌ No se encontró el elemento <audio id='song'>");
      return;
    }
    ...
    if (gl) {
      ...
      function drawGL() {
        ...
        if (body.classList.contains('epic')) {
    ...
    function epicLoop() {
      ...
      if (inEpic) {
        ...
        // Disparar flares en los compases correctos
        ...
        if (t < EPIC_START) nextCompas = 0; // reset si rebobina antes del segmento
      }
